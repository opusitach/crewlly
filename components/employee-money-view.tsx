...
export default function EmployeeMoneyView({ onBack, employeeId, employeeName }: Props) {
  ...
  useEffect(() => {
    const loadItems = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/employees/${employeeId}/earnings?${params.toString()}`, { credentials: "include" })
        if (!res.ok) {
          setItems([])
          setSummary({
            totalGrossCents: 0,
    ...
    void loadItems()
  }, [appliedRange.fromDate, appliedRange.toDate, employeeId])
